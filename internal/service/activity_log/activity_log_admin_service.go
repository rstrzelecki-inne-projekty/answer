...
func (s *ActivityLogAdminService) Daily(ctx context.Context, req *schema.ActivityLogDailyReq) ([]*schema.ActivityLogDailyRow, error) {
	...
	rows, err := s.repo.ListActionsBetween(ctx, start, time.Time{})
	if err != nil {
		return nil, err
	}
...
// the other columns are shown for context only. [cd] The aggregation lives in ActivityLogService so that
// the public user ranking can reuse it without depending on the admin service.
func (s *ActivityLogAdminService) TopUsers(ctx context.Context, req *schema.ActivityLogTopUsersReq) ([]*schema.ActivityLogTopUserRow, error) {
	return s.logService.TopUsers(ctx, req)
}
